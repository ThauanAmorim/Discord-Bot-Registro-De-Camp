...
exports.run = async (bot, message = new Message, args) => {
    ...
    try {
        ...
        if (author.roles.cache.has(idStaff)) {
            if (args.length < 1) {
                message.channel.send(`Digite o comando e em seguinda mencione os membros para receber o cargo ${cargoTeamLeader.name}! ex.: /AdicionarCapitao @membro1 @membro2 @membro3`)
            } else {
            ...
            message.channel.send(`Para adicionar o cargo de ${cargoTeamLeader.name} você deve ter o cardo de ${cargoAdm.name}!`)
        }
    } catch (e) {
